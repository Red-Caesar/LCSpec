...
def get_dataframe(
    ...
    *,
    ...
    load_dotenv()
    resolved_conn_str = connection_string or os.getenv("SUPABASE_DB_URL")

    if resolved_conn_str:
        conn = psycopg2.connect(resolved_conn_str)
    ...
    raise ValueError(
        "Provide db_path, connection_string, or set SUPABASE_DB_URL env var."
    )
...
def add_lt_setup_type(df: pd.DataFrame) -> pd.DataFrame:
    df["setup_type"] = df.apply(
        lambda row: (
            f"{row['sd_method_type']}_{row['sd_model_name']}_sd_tokens={row['num_spec_tokens']}"
            if row["sd_model_name"] != ""
...
def add_sd_setup_type(df: pd.DataFrame) -> pd.DataFrame:
    df["setup_type"] = df.apply(
        lambda row: (
            f"{row["sd_method_type"]}_{row["sd_model_name"].split("/")[-1]}"
        ),
        axis=1,
    )
    return df
...
def plot_2d_rates_vs_context(
    df: pd.DataFrame | None,
    dataset_type: str,
    model_name: str,
    metric: str = "mean_acceptance_length",
):
    dataset = df[
        (df.dataset_type == dataset_type)
        & (df.target_model_name == model_name)
    ...
    ax.set_title(
        f"{metric.replace('_', ' ').title()} vs Context for {model_name}\n Dataset {dataset_type}", fontsize=14
    )
