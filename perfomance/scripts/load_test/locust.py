...
from perfomance.scripts.load_test.custom_datasets import DatasetFactory
...
def on_request(request_type, name, response_time, response_length, exception, **kwargs):
    ...
    if test_start_time and test_duration_seconds:
        ...
        if current_time - last_progress_time >= 1:
            ...
            median_out = (
                f"{int(np.median(output_token_counts))}"
                if output_token_counts
                else "n/a"
            )
            print(
                f"\rProgress: {progress:.1f}% | Elapsed: {elapsed_str} | Remaining: {remaining_str} | median_out_tokens: {median_out} ",
                end="",
                flush=True,
            )
...
class LLMInferenceTasks(TaskSet):
    def on_start(self):
        ...
        if distribution == "const":
            self._max_tokens_fn = lambda: int(params)
        elif distribution == "normal":
            mean, std = list(map(int, params.split(",")))
            self._max_tokens_fn = lambda: max(1, int(np.random.normal(mean, std)))
        else:
    ...
    def generate_text(self):
        ...
        request_payload = {
            ...
            "max_tokens": self._max_tokens_fn(),
            "temperature": 0.0,
            "stream": False,
        }
...
def on_test_stop(environment: Environment, **kwargs):
    ...
    if environment.runner and hasattr(environment.runner, "stats"):
        ...
        if stats.entries:
            ...
            out_arr = np.array(output_token_counts) if output_token_counts else None
            locust_stats = {
                "summary": {
                    ...
                    "output_tokens": {
                        "median": int(np.median(out_arr)) if out_arr is not None else None,
                        "min": int(out_arr.min()) if out_arr is not None else None,
                        "max": int(out_arr.max()) if out_arr is not None else None,
                        "p90": int(np.percentile(out_arr, 90)) if out_arr is not None else None,
                        "p95": int(np.percentile(out_arr, 95)) if out_arr is not None else None,
                        "p99": int(np.percentile(out_arr, 99)) if out_arr is not None else None,
                    },
